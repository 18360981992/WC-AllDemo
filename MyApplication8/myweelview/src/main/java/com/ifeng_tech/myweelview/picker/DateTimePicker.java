...
import com.ifeng_tech.myweelview.util.DateUtils;
...
//    }
    ...
    public void setSelectedItem(int year, int month, int day, int hour, int minute) {
        ...
        LogUtils_weel.verbose(this, "change months and days while set selected");
        ...
        if (timeMode != NONE) {
            ...
            if ( hours.size() == 0) {
                LogUtils_weel.verbose(this, "init hours before make view");
                initHourData();
            }
    ...
    public void setSelectedItem(int yearOrMonth, int monthOrDay, int hour, int minute) {
        ...
        if (dateMode == MONTH_DAY) {
            ...
            LogUtils_weel.verbose(this, "change months while set selected");
    ...
    protected View makeCenterView() {
        ...
        if ((dateMode == YEAR_MONTH_DAY || dateMode == YEAR_MONTH) && years.size() == 0) {
            LogUtils_weel.verbose(this, "init years before make view");
            initYearData();
        }
        if (dateMode != NONE && months.size() == 0) {
            LogUtils_weel.verbose(this, "init months before make view");
        ...
        if ((dateMode == YEAR_MONTH_DAY || dateMode == MONTH_DAY) && days.size() == 0) {
            LogUtils_weel.verbose(this, "init days before make view");
            int selectedYear;
            if (dateMode == YEAR_MONTH_DAY) {
        ...
        if (timeMode != NONE && minutes.size() == 0) {
            LogUtils_weel.verbose(this, "init minutes before make view");
            changeMinuteData(DateUtils.trimZero(selectedHour));
        }
            ...
            if (dateMode == YEAR_MONTH_DAY || dateMode == YEAR_MONTH) {
                ...
                yearView.setOnItemPickListener(new OnItemPickListener<String>() {
                    ...
                    public void onItemPicked(int index, String item) {
                        ...
                        LogUtils_weel.verbose(this, "change months after year wheeled");
...
//                        }
                        if (dateMode == YEAR_MONTH_DAY || dateMode == MONTH_DAY) {
                            LogUtils_weel.verbose(this, "change days after month wheeled");
                            selectedDayIndex = 0;//重置日子索引
                            int selectedYear;
                            if (dateMode == YEAR_MONTH_DAY) {
                                selectedYear = DateUtils.trimZero(getSelectedYear());
...
//                        LogUtils_weel.verbose(this, "change minutes after hour wheeled");
                        changeMinuteData(DateUtils.trimZero(item));
