...
public class PieChartRenderer extends DataRenderer {
    ...
    public PieChartRenderer(PieChart chart, ChartAnimator animator,
        ...
        mEntryLabelsPaint.setTextSize(Utils.convertDpToPixel(18f));
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            for (int j = 0; j < entryCount; j++) {
                ...
                if (drawXOutside || drawYOutside) {
                    ...
                    /**
                     * 这里是绘制圈外线所以须添加  根据文字内容改变线的颜色
                     *
                     * 硬编码  根据字符串判断 强行更改画笔颜色
                     */
                    if (entry.isDisplay()) {
                        if (dataSet.getValueLineColor() != ColorTemplate.COLOR_NONE) {
                            if(entry.getLabel().contains(ColorTemplate.ST_zhuan)){
                                mValueLinePaint.setColor(Color.parseColor(ColorTemplate.COLOR_zhuan));
                            }else if(entry.getLabel().contains(ColorTemplate.ST_cang)){
                                mValueLinePaint.setColor(Color.parseColor(ColorTemplate.COLOR_cang));
                            }else if(entry.getLabel().contains(ColorTemplate.ST_jian)){
                                mValueLinePaint.setColor(Color.parseColor(ColorTemplate.COLOR_jian));
                            }else{
                                mValueLinePaint.setColor(Color.parseColor(ColorTemplate.COLOR_ti));
                            }
    ...
    protected void drawEntryLabel(Canvas c, String label, float x, float y) {
        if(label.contains(ColorTemplate.ST_zhuan)){
            mEntryLabelsPaint.setColor(Color.parseColor(ColorTemplate.COLOR_zhuan));
        }else if(label.contains(ColorTemplate.ST_cang)){
            mEntryLabelsPaint.setColor(Color.parseColor(ColorTemplate.COLOR_cang));
        }else if(label.contains(ColorTemplate.ST_jian)){
            mEntryLabelsPaint.setColor(Color.parseColor(ColorTemplate.COLOR_jian));
        }else{
            mEntryLabelsPaint.setColor(Color.parseColor(ColorTemplate.COLOR_ti));
        }
