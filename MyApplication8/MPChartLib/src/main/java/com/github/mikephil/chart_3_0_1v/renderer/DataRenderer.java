...
public abstract class DataRenderer extends Renderer {
    ...
    public void drawValue(Canvas c, IValueFormatter formatter, float value, Entry entry, int dataSetIndex, float x, float y, int color) {
        ...
        String label = entry1.getLabel();
        if(label.contains(ColorTemplate.ST_zhuan)){
            mValuePaint.setColor(Color.parseColor(ColorTemplate.COLOR_zhuan));
        }else if(label.contains(ColorTemplate.ST_cang)){
            mValuePaint.setColor(Color.parseColor(ColorTemplate.COLOR_cang));
        }else if(label.contains(ColorTemplate.ST_jian)){
            mValuePaint.setColor(Color.parseColor(ColorTemplate.COLOR_jian));
        }else{
            mValuePaint.setColor(Color.parseColor(ColorTemplate.COLOR_ti));
        }
...
//        String valueTemp = formatter.getFormattedValue(value, entry, dataSetIndex, mViewPortHandler);  // 计算出百分比

        String valueTemp=entry1.getValue()+"";  // 将百分比替换成数量
